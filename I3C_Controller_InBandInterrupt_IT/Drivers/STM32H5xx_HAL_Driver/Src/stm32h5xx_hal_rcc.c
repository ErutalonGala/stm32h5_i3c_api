...
HAL_StatusTypeDef HAL_RCC_DeInit(void) {
	...
	CLEAR_BIT(RCC->CR,
			RCC_CR_CSION | RCC_CR_CSIKERON | RCC_CR_HSECSSON | RCC_CR_HSIKERON | RCC_CR_HSI48ON | RCC_CR_HSEON);
...
void HAL_RCC_MCOConfig(uint32_t RCC_MCOx, uint32_t RCC_MCOSource,
	...
	if (RCC_MCOx == RCC_MCO1) {
		...
		MODIFY_REG(RCC->CFGR1, (RCC_CFGR1_MCO1SEL | RCC_CFGR1_MCO1PRE),
		...
		MODIFY_REG(RCC->CFGR1, (RCC_CFGR1_MCO2SEL | RCC_CFGR1_MCO2PRE),
...
uint32_t HAL_RCC_GetPCLK1Freq(void) {
	...
	return (HAL_RCC_GetHCLKFreq()
			>> ((APBPrescTable[(RCC->CFGR2 & RCC_CFGR2_PPRE1)
					>> RCC_CFGR2_PPRE1_Pos]) & 0x1FU));
}
...
uint32_t HAL_RCC_GetPCLK2Freq(void) {
	...
	return (HAL_RCC_GetHCLKFreq()
			>> ((APBPrescTable[(RCC->CFGR2 & RCC_CFGR2_PPRE2)
					>> RCC_CFGR2_PPRE2_Pos]) & 0x1FU));
}
...
uint32_t HAL_RCC_GetPCLK3Freq(void) {
	...
	return (HAL_RCC_GetHCLKFreq()
			>> ((APBPrescTable[(RCC->CFGR2 & RCC_CFGR2_PPRE3)
					>> RCC_CFGR2_PPRE3_Pos]) & 0x1FU));
}
/**
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *pOscInitStruct) {
	...
	pOscInitStruct->OscillatorType = RCC_OSCILLATORTYPE_HSE
	...
	pOscInitStruct->HSEState = (regval
			& (RCC_CR_HSEON | RCC_CR_HSEBYP | RCC_CR_HSEEXT));
	...
	pOscInitStruct->LSEState = (regval
			& (RCC_BDCR_LSEON | RCC_BDCR_LSEBYP | RCC_BDCR_LSEEXT));
	...
	pOscInitStruct->PLL.PLLM = (uint32_t) ((reg1val & RCC_PLL1CFGR_PLL1M)
			>> RCC_PLL1CFGR_PLL1M_Pos);
	pOscInitStruct->PLL.PLLN = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1N)
			>> RCC_PLL1DIVR_PLL1N_Pos) + 1U);
	pOscInitStruct->PLL.PLLQ = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1Q)
			>> RCC_PLL1DIVR_PLL1Q_Pos) + 1U);
	pOscInitStruct->PLL.PLLR = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1R)
			>> RCC_PLL1DIVR_PLL1R_Pos) + 1U);
	pOscInitStruct->PLL.PLLP = (uint32_t) (((reg2val & RCC_PLL1DIVR_PLL1P)
			>> RCC_PLL1DIVR_PLL1P_Pos) + 1U);
	pOscInitStruct->PLL.PLLRGE = (uint32_t) ((reg1val & RCC_PLL1CFGR_PLL1RGE));
	pOscInitStruct->PLL.PLLVCOSEL = (uint32_t) ((reg1val
			& RCC_PLL1CFGR_PLL1VCOSEL) >> RCC_PLL1CFGR_PLL1VCOSEL_Pos);
	pOscInitStruct->PLL.PLLFRACN = (uint32_t) (((RCC->PLL1FRACR
			& RCC_PLL1FRACR_PLL1FRACN) >> RCC_PLL1FRACR_PLL1FRACN_Pos));
}
 ...
 * @retval None
 */
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef *pClkInitStruct,
		uint32_t *pFLatency) {
	uint32_t regval;
	...
	pClkInitStruct->ClockType = RCC_CLOCKTYPE_SYSCLK | RCC_CLOCKTYPE_HCLK
	...
	pClkInitStruct->APB2CLKDivider =
			(uint32_t) ((regval & RCC_CFGR2_PPRE2) >> 4);

	/* Get the APB3 configuration ----------------------------------------------*/
	pClkInitStruct->APB3CLKDivider =
			(uint32_t) ((regval & RCC_CFGR2_PPRE3) >> 8);
 ...
 */
HAL_StatusTypeDef HAL_RCC_GetConfigAttributes(uint32_t Item,
  ...
  attributes = ((RCC->PRIVCFGR & RCC_PRIVCFGR_NSPRIV) == 0U) ? RCC_NSEC_NPRIV : RCC_NSEC_PRIV;
	...
	attributes =
			((RCC->PRIVCFGR & RCC_PRIVCFGR_PRIV) == 0U) ? RCC_NPRIV : RCC_PRIV;
#endif /* RCC_SECCFGR_HSISEC */
