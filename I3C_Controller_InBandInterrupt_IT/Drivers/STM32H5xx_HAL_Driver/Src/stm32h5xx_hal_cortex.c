 ...
 * @{
 */
static void MPU_ConfigRegion(MPU_Type *MPUx,
		const MPU_Region_InitTypeDef *const pMPU_RegionInit);
static void MPU_ConfigMemoryAttributes(MPU_Type *MPUx,
...
static void MPU_ConfigRegion(MPU_Type *MPUx,
  ...
  assert_param(IS_MPU_INSTANCE(MPUx));
	...
	if (pMPU_RegionInit->Enable != MPU_REGION_DISABLE) {
		...
		assert_param(
				IS_MPU_REGION_PERMISSION_ATTRIBUTE(pMPU_RegionInit->AccessPermission));
		...
		MPUx->RBAR = (((uint32_t) pMPU_RegionInit->BaseAddress & 0xFFFFFFE0UL)
				...
				| ((uint32_t) pMPU_RegionInit->AccessPermission
						<< MPU_RBAR_AP_Pos)
...
static void MPU_ConfigMemoryAttributes(MPU_Type *MPUx,
  ...
  assert_param(IS_MPU_INSTANCE(MPUx));
	...
	*(p_mair) =
			attr_values
					| ((uint32_t) pMPU_AttributesInit->Attributes
							<< (attr_number * 8U));
}
/**
 * @}
