...
static void DMA_SetConfig(DMA_HandleTypeDef const *const hdma,
		uint32_t SrcAddress, uint32_t DstAddress, uint32_t SrcDataSize);
...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *const hdma) {
	...
	if (IS_GPDMA_INSTANCE(hdma->Instance) != 0U) {
		...
		assert_param(
				IS_DMA_TRANSFER_ALLOCATED_PORT(hdma->Init.TransferAllocatedPort));
	}
...
HAL_StatusTypeDef HAL_DMA_PollForTransfer(DMA_HandleTypeDef *const hdma,
	...
	level_flag = (
			(CompleteLevel == HAL_DMA_FULL_TRANSFER) ?
					DMA_FLAG_IDLE : DMA_FLAG_HT);
...
void HAL_DMA_IRQHandler(DMA_HandleTypeDef *const hdma) {
	...
	uint32_t global_active_flag_ns = IS_DMA_GLOBAL_ACTIVE_FLAG_NS(
			p_dma_instance, global_it_flag);
#if defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
 ...
 * @retval HAL Status.
 */
HAL_StatusTypeDef HAL_DMA_GetConfigChannelAttributes(
		DMA_HandleTypeDef const *const hdma, uint32_t *const pChannelAttributes) {
	...
	attributes =
			((p_dma_instance->PRIVCFGR & channel_idx) == 0U) ?
					DMA_CHANNEL_NPRIV : DMA_CHANNEL_PRIV;
...
static void DMA_Init(DMA_HandleTypeDef const *const hdma) {
	...
	MODIFY_REG(hdma->Instance->CCR, DMA_CCR_PRIO | DMA_CCR_LAP | DMA_CCR_LSM,
			tmpreg);

	/* Prepare DMA Channel Transfer Register (CTR1) value ***************************************************************/
	tmpreg = hdma->Init.DestInc | hdma->Init.DestDataWidth | hdma->Init.SrcInc
			| hdma->Init.SrcDataWidth;

	/* Add parameters specific to GPDMA */
	if (IS_GPDMA_INSTANCE(hdma->Instance) != 0U) {
		tmpreg |= (hdma->Init.TransferAllocatedPort
				| (((hdma->Init.DestBurstLength - 1U) << DMA_CTR1_DBL_1_Pos)
						& DMA_CTR1_DBL_1)
  ...
  MODIFY_REG(hdma->Instance->CTR1, ~(DMA_CTR1_SSEC | DMA_CTR1_DSEC), tmpreg);
	...
	tmpreg = hdma->Init.BlkHWRequest | (hdma->Init.Request & DMA_CTR2_REQSEL)
	...
	MODIFY_REG(hdma->Instance->CTR2,
			(DMA_CTR2_TCEM | DMA_CTR2_TRIGPOL | DMA_CTR2_TRIGSEL | DMA_CTR2_TRIGM | DMA_CTR2_PFREQ | DMA_CTR2_BREQ | DMA_CTR2_DREQ | DMA_CTR2_SWREQ | DMA_CTR2_REQSEL),
			tmpreg);
